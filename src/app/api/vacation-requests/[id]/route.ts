...
// PATCH /api/vacation-requests/[id] — approve, deny, or cancel
export async function PATCH(
  ...
  // Physicians can only cancel their own requests
  if (!isAdmin) {
    if (status !== "CANCELLED") {
  ...
  const actionMap: Record<string, string> = {
    APPROVED: "APPROVE_VACATION",
    DENIED: "DENY_VACATION",
    CANCELLED: "CANCEL_VACATION",
  };
  await auditLog(
    ...
    "VacationRequest",
    id,
    { previousStatus: request.status, newStatus: status, reviewNote }
  );
